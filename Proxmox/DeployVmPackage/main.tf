...
resource "proxmox_vm_qemu" "test_server" {
  ...
  name = "debian-vm-1" #count.index starts at 0, so + 1 means this VM will be named debian-vm-1 in proxmox
  ...
  ipconfig0 = "ip=192.168.100.91/24,gw=192.168.100.1"
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      connection {
      type = "ssh"
      #host     = proxmox_vm_qemu.test_server[count.index].network_interface.0.ip
      host        = "192.168.100.91"
      user        = "root"
